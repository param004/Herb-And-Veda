...
export default function CheckoutConfirmationPage() {
  ...
  const handleConfirmOrder = async () => {
    // Require login/token to place order
    if (!token) {
      navigate('/login', { state: { from: '/checkout-confirmation' } });
      return;
    }

    if (!user?.phone || !user?.address) {
      showToast && showToast('Please complete your personal information before checkout', { type: 'error' });
    ...
    try {
      ...
      if (!newOrderId) {
        ...
        showToast && showToast('Order created, but we could not retrieve the order id. Please check your order history.', { type: 'info' });
        navigate('/orders');
        return;
      }
      ...
      showToast && showToast(error.response?.data?.message || 'Failed to place order. Please try again.', { type: 'error' });
    } finally {
      setLoading(false);
    }
  };
